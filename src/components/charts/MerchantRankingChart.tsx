...
function MerchantChart({
  ...
  const visible: MerchantAmount[] = showAll
    ? data
    : hasMore
      ? [...top, { merchant: OTHER_KEY, amount: otherAmount }]
      : top

  // Fit Y-axis to the longest label (approx 7px per char at font-size 11)
  const yAxisWidth = Math.min(160, Math.max(90,
    ...visible.map((e) =>
      (e.merchant === OTHER_KEY ? `Other (${rest.length})` : e.merchant).length * 7
    )
  ))

  const handleClick = (entry: { merchant: string }) => {
    if (entry.merchant === OTHER_KEY) return
      ...
      <ResponsiveContainer width="100%" height={Math.max(200, visible.length * 36)}>
        ...
        >
          ...
          <Tooltip
            ...
            labelFormatter={(label) => label === OTHER_KEY ? `Other (${rest.length} merchants)` : label}
          ...
          <Bar dataKey="amount" radius={[0, 4, 4, 0]} cursor="pointer">
            ...
            {visible.map((entry) => (
              <Cell
                key={entry.merchant}
                fill={entry.merchant === OTHER_KEY ? '#C8BFB8' : color}
                opacity={
                  entry.merchant === OTHER_KEY
                    ? 0.7
                    : !selected || selected === entry.merchant
                      ? 1
                      : 0.3
                }
              />
